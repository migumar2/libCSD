/* StringDictionaryRPFC.h
 * Copyright (C) 2014, Francisco Claude & Rodrigo Canovas & Miguel A. Martinez-Prieto
 * all rights reserved.
 *
 * This class implements a Compressed String Dictionary which differentially
 * encodes the strings using (Plain) Front-Coding [1] and performs Re-Pair [2]
 * compression on the internal strings.
 * 
 *   ==========================================================================
...
#ifndef _STRINGDICTIONARY_RPFC_H
#define _STRINGDICTIONARY_RPFC_H
...
class StringDictionaryRPFC : public StringDictionary
{
	public:
		/** Generic Constructor. */
		StringDictionaryRPFC();

		/** Class Constructor.
		    @param it: iterator scanning the original set of strings.
		    @param bucketsize: number of strings represented per bucket.
		*/
		StringDictionaryRPFC(IteratorDictString *it, uint bucketsize);


		StringDictionaryRPFC(uchar *file);
	...
	protected:
		...
		*/
		inline uint searchPrefix(uchar **ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen, uint *offset);
		...
		/** Searches the first string non-prefixed for the given one.
		    ...
		    @returns the internal ID (of 0 if it is not in the bucket).
		*/
		inline uint searchDistinctPrefix(uchar *ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen, uint *offset);
...
#endif  /* _STRINGDICTIONARY_RPFC_H */
