...
namespace cds_static{
	...
	RMQ_succinct_lcp::RMQ_succinct_lcp(LCP *lcp, TextIndex *csa) {
		...
		for (uint i = 0; i < nmb; i++){ // step through microblocks
			start = z;            // init start
			...
			rp[1] = lcp->get_LCP(z, csa); // init rightmost path
			while (++z < end){   // step through current block:
				...
				rp[q-p] = lcp->get_LCP(z, csa); // add last element to rightmost path
			}
		...
		for (uint i = 0; i < nb; i++) { // step through blocks
			start = z;              // init start
			p = start;              // init minimum
			end = start + sprime;   // end of block (not inclusive!)
			if (end > n) end = n;   // last block could be smaller than sprime!
			if (lcp->get_LCP(z,csa) < lcp->get_LCP(q,csa)) q = z; // update minimum in superblock
		
			while (++z < end) { // step through current block:
				if (lcp->get_LCP(z,csa) < lcp->get_LCP(p,csa)) p = z; // update minimum in block
				if (lcp->get_LCP(z,csa) < lcp->get_LCP(q,csa)) q = z; // update minimum in superblock
			}
		...
		for (uint j = 1; j < Mprime_depth; j++) {
			...
			for (uint i = nsb - dist; i < nsb; i++) Mprime[j][i] = Mprime[j-1][i]; // overhang
			dist *= 2;
		}	   
	}
	...
	uint RMQ_succinct_lcp::query(uint i, uint j,  TextIndex *csa, LCP *lcp) {
		uint mb_i = microblock(i);     // i's microblock
		uint mb_j = microblock(j);     // j's microblock
		...
		if (mb_i == mb_j) { // only one microblock-query
			...
			uint j_pos = j - s_mj;    // position of j in its microblock
			...
			if (mb_j > mb_i + 1) { // otherwise we're done!
				uint s_bi = b_i * sprime;      // start of block i
				uint s_bj = b_j * sprime;      // start of block j	
				...
				if (b_j > b_i + 1) { // otherwise no out-of-block-queries
					...
					if (s_bj - s_bi - sprime <= sprimeprime) { // just one out-of-block-query
						...
						uint sb_i = superblock(i); // i's superblock
						uint sb_j = superblock(j); // j's superblock
